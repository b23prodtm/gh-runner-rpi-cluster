...
if [[ -f "${TOPDIR}/${SERVICE}/x86_64.env" ]]; then
    ln -sf "${TOPDIR}/${SERVICE}/x86_64.env" "${CURRENT_DIR}/.env"
    echo "✅ Symlink créé: .env → ${SERVICE}/x86_64.env"
elif [[ -f "${TOPDIR}/x86_64.env" ]]; then
    ln -sf "${TOPDIR}/x86_64.env" "${CURRENT_DIR}/.env"
    echo "✅ Symlink créé: .env → x86_64.env"
else
    echo "⚠️  Pas de fichier x86_64.env trouvé"
fi
