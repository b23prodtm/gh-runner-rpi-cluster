...
if [[ -f "${TOPDIR}/${SERVICE}/armhf.env" ]]; then
    ln -sf "${TOPDIR}/${SERVICE}/armhf.env" "${CURRENT_DIR}/.env"
    echo "✅ Symlink créé: .env → ${SERVICE}/armhf.env"
elif [[ -f "${TOPDIR}/armhf.env" ]]; then
    ln -sf "${TOPDIR}/armhf.env" "${CURRENT_DIR}/.env"
    echo "✅ Symlink créé: .env → armhf.env"
else
    echo "⚠️  Pas de fichier armhf.env trouvé"
fi
